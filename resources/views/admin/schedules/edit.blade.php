...
@section('page-subtitle','Perbarui informasi slot waktu.')
@section('content')
            ...
            <div class="space-y-5">
                    ...
                    <select name="field_id" required
                        ...
                        @foreach($fields as $field)
                            <option value="{{ $field->id_fields }}"
                                {{ old('field_id', $schedule->field_id) == $field->id_fields ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="status_schedules"
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary bg-white
                            {{ $schedule->status_schedules === 'booked' ? 'opacity-60' : '' }}">
                        <option value="available" {{ old('status_schedules', $schedule->status_schedules) === 'available' ? 'selected' : '' }}>Available</option>
                        <option value="booked" {{ old('status_schedules', $schedule->status_schedules) === 'booked' ? 'selected' : '' }}>Booked</option>
                        <option value="closed" {{ old('status_schedules', $schedule->status_schedules) === 'closed' ? 'selected' : '' }}>Closed</option>
                    </select>
                    @if($schedule->status_schedules === 'booked')
                    <p class="text-xs text-yellow-600 mt-1"><i class="fas fa-exclamation-triangle mr-1"></i>Jadwal ini sudah dibooking. Ubah dengan hati-hati.</p>
                    @endif
                </div>
            </div>
